...
For example, reading a file called 128Mb+1 will return a file of 128 Megabytes
minus 1 byte
...
class Filler(object):
    ...
    def __init__(self, choices=None, random=False):
        ...
        self.random = random
        if not choices:
            self.choices = string.ascii_lowercase + string.digits
...
class SizeFS(FS):  # pylint: disable=R0902,R0904,R0921
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ones = DirEntry('dir', 'ones', filler=Filler(choices='1'))
        self.random = DirEntry('dir', 'random', filler=Filler(random=True))
        self.common = DirEntry('dir', 'common')

        for filename in files:
            self.zeros.contents[filename] = DirEntry(
                'file', filename, filler=Filler(choices='0'))
            self.ones.contents[filename] = DirEntry(
                'file', filename, filler=Filler(choices='1'))
            self.random.contents[filename] = DirEntry(
                'file', filename, filler=Filler(random=True))
